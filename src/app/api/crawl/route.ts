  ...
  try {
    ...
    if (!finalApiKey || finalApiKey === "fc-YOUR_API_KEY") {
      return NextResponse.json(
        {
          error:
            "Firecrawl API key not configured. Please check server configuration.",
        },
        { status: 500 },
      );
    }
    ...
    if (!scrapeResult.success) {
      ...
      if (
        ...
        return NextResponse.json(
          {
            error:
              "The website is temporarily unavailable. Please try again later or try a different URL.",
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const scrapeOptions: ScrapeOptions = {
      formats: ["markdown", "json"], // Always get both formats for maximum flexibility
      onlyMainContent: true,
      timeout: 60000, // 60 second timeout per URL
      maxAge: maxAge,
    };

    // Add excludePaths if provided (for potential future Firecrawl support)
    if (excludePaths && excludePaths.length > 0) {
      (scrapeOptions as any).excludePaths = excludePaths;
    }
    ...
    try {
      ...
      const batchScrapeResult = await app.batchScrapeUrls(
        selectedUrls,
        scrapeOptions as Parameters<typeof app.batchScrapeUrls>[1],
      );

      if (!batchScrapeResult.success) {
        throw new Error(
          "error" in batchScrapeResult
      ...
      const resultData =
        "data" in batchScrapeResult ? batchScrapeResult.data || [] : [];
      crawlResult = {
        success: true,
        data: resultData,
        ...
        creditsUsed:
          "creditsUsed" in batchScrapeResult
            ? batchScrapeResult.creditsUsed
            : resultData.length,
      };
    } catch (error) {
      ...
      if (
        ...
        return NextResponse.json(
          {
            error:
              "Insufficient Firecrawl API credits. Please check your account balance at https://app.firecrawl.dev or upgrade your plan.",
            details:
              "The batch scrape operation requires more credits than available in your account.",
          },
          { status: 402 },
        );
      }
      ...
      for (const url of selectedUrls) {
        try {
          const result = await app.scrapeUrl(
            url,
            scrapeOptions as Parameters<typeof app.scrapeUrl>[1],
          );
              ...
              url: String(
                ("url" in result ? result.url : "") ||
                  ("metadata" in result && result.metadata
                    ? (result.metadata as Record<string, unknown>)?.sourceURL
                    : "") ||
                  "",
              ),
              links: "links" in result ? result.links || [] : [],
              screenshot:
                "screenshot" in result ? result.screenshot : undefined,
            };
          }),
        total: selectedUrls.length,
      ...
      for (const [key, property] of Object.entries(finalSchema.properties)) {
        if (key === "title") {
          ...
          structuredData.title =
            page.metadata?.title ||
            extractTitleFromMarkdown(page.markdown || "");
        } else if (key === "date") {
          // Try to extract date from metadata or content
          structuredData.date =
            page.metadata?.publishedTime ||
            extractDateFromMarkdown(page.markdown || "");
        } else if (key === "content") {
          ...
          structuredData[key] = extractFieldFromMarkdown(
            page.markdown || "",
            key,
      ...
      response.rawData =
        crawlResult.data?.map((page) => ({
          ...
          markdown:
            ...
            (page.markdown && page.markdown.length > 2000 ? "..." : ""),
...
function inferSchemaFromContent(data: {
  markdown?: string;
  html?: string;
}): Schema {
  ...
  const patterns = [
    ...
    },
    {
      ...
      field: "image_url",
      type: "string" as const,
      description: "Image URL from markdown",
    },
    ...
    // Rating patterns
    {
      regex: /rating[:\s]*(\d+\.?\d*)/gi,
      field: "rating",
      type: "number" as const,
      description: "Rating score",
    },
    {
      regex: /(\d+\.?\d*)\s*stars?/gi,
      field: "rating",
      type: "number" as const,
    ...
    {
      regex: /by\s+([A-Z][a-z]+\s+[A-Z][a-z]+)/g,
      field: "author",
      type: "string" as const,
  ...
  if (content.toLowerCase().includes("product") || schema.properties.price) {
    ...
    if (!schema.properties.availability)
  ...
  if (
    ...
    if (!schema.properties.tags)
      schema.properties.tags = { type: "array", description: "Article tags" };
  }
  ...
  if (
    ...
    if (!schema.properties.name)
      schema.properties.name = {
        type: "string",
        description: "Business or person name",
      };
    if (!schema.properties.phone)
      schema.properties.phone = { type: "string", description: "Phone number" };
...
function extractFieldFromMarkdown(
  markdown: string,
  fieldName: string,
  fieldType: string,
): unknown {
